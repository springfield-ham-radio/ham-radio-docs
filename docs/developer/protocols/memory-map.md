...
- **Settings groups:** declare a top-level `groups` array. Each group has `id` and `label`, and may include `description`, `icon` (Iconify name), and `warning` (`title` + `description`). A radio-wide field’s `ui.group` must match a group `id`. Declaration order is the left-nav order on the Settings tab.
...
- **Per-channel extras:** put `ui` on fields of the `channelBindings.records` struct and set `ui.group` to `channel`. That id is not a settings group and is not declared in `groups`. `collectChannelMemoryMapUiFields` returns those fields for Channels-table columns. Bound RadioChannel fields (`rxfreq`, tones, …) are omitted. Use `formatMemoryMapFieldValue` for read-only cell text.
